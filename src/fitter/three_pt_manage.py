...
import cmath
from fit_collect import fitter
...
class fit_ensemble(object):
    ...
    def __init__(self, t_range, prior, n_states=None,
        ...
        for data_gv in [nucleon_corr_gv, axial_fh_num_gv, vector_fh_num_gv]:
            if data_gv is not None:
                t_max = 25
                # t_max = len(data_gv[data_gv.keys()[0]])
    ...
    def _get_models(self, model_type=None):
        ...
        return fitter(n_states=self.n_states, prior=self.prior, t_range=self.t_range,
                      nucleon_corr_gv=nucleon_corr_gv, axial_fh_num_gv=axial_fh_num_gv,
    ...
    def get_nucleon_effective_mass(self, nucleon_corr_gv=None, dt=None):
        ...
        return {key : 1/dt * np.log(nucleon_corr_gv[key] / np.roll(nucleon_corr_gv[key], -1))
                for key in nucleon_corr_gv.keys()}
    ...
    def get_nucleon_effective_wf(self, nucleon_corr_gv=None, t=None, dt=None):
        ...
        if t is None:
            t = {key : np.arange(len(nucleon_corr_gv[key])) for key in nucleon_corr_gv.keys()}
        else:
            t = {key : t for key in nucleon_corr_gv.keys()}
    ...
    def plot_effective_mass(self, nucleon_corr_gv=None, t_plot_min=None,
                            t_plot_max=None, show_plot=False, show_fit=True):
        if t_plot_min is None:
            t_plot_min = self.t_min
        if t_plot_max is None:
