...
class eff_plots():
    ''' 
    Initialize axis dictionaries to be used globally. 
    '''
    def __init__(self):
        self.ax_meff = {}
        self.ax_zeff = {}
    ...
    def make_eff_plots(self,states,fp,x_fit,priors,gv_data,fit, scale,show_fit,save_figs):
        ...
        if 'mres' not in k:
            ...
            eff_plots.plot_mres(self.ax_meff[k], gv_data, k,
                            mtype=fp.corr_lst[k]['type'], colors=clrs, offset=t0)
        ...
        if 'mres' not in k:
            if 'denom' in fp.corr_lst[k]:
                ...
                self.ax_r[k].axhspan(p.mean-p.sdev, p.mean
                                + p.sdev, color='k', alpha=.2)
                plot_eff(self.ax_r[k], gv_data, k, mtype=fp.corr_lst[k]['type'])
            ...
            eff_plots.plot_zeff(self.ax_zeff[k], gv_data, k, ztype=ztype,
    ...
    def plot_eff(ax, dsets, key, mtype='exp', tau=1, colors=None, offset=0, denom_key=None):
        ...
        for k in lst:
            ...
            lbl = k.split('_')[1]
            eff = eff_plots.effective_mass(data, mtype=mtype, tau=tau)
    ...
    def plot_zeff(ax, dsets, key, ztype='A_snk,src', snksrc=None, mtype='exp', tau=1, colors=None):
        ...
        if ztype == 'A_snk,src':
            ...
            for snk in snksrc['snks']:
                ...
                k = key+'_'+snk+src
                lbl = r'$z_{\rm %s}$' % snk
                eff = eff_plots.effective_mass(dsets[k], mtype=mtype, tau=tau)
                t = np.arange(eff.shape[0])
                ...
                if snk == src:
                    ...
                    k2 = key+'_'+src+src
                    eff2 = eff_plots.effective_mass(dsets[k2], mtype=mtype, tau=tau)
                    if 'exp' in mtype:
    ...
    # def plot_mres(ax, dsets,fp, key, priors, mtype='exp', tau=1, colors=None, offset=0, denom_key=None,show_fit = True):
    #     lst      = [k for k in dsets if key in k]
    ...
    #     if colors is not None:
    #         ax.fill_between(np.arange(10,Nt/2+1,.1), mres_avg.mean-mres_avg.sdev, mres_avg.mean+mres_avg.sdev, 
    #                         color=colors,alpha=.3, label='avg')
    #     else:
    #         ax.fill_between(np.arange(10,Nt/2+1,.1), mres_avg.mean-mres_avg.sdev, mres_avg.mean+mres_avg.sdev, 
    #                         alpha=.3, label='avg')
    ...
    #         y_fit = mres[x_fit]
    #         fit = lsqfit.nonlinear_fit(data=(x_fit, y_fit), prior=p, fcn=fit_funcs.corr_functions.mres(x, p), svdcut=svdcut)
    #         print(fit)

    #         if colors is not None:
    #             ax.fill_between(x_fit, fit.p['mres'].mean-fit.p['mres'].sdev, fit.p['mres'].mean+fit.p['mres'].sdev, 
    #                             color=colors,alpha=.3,label='fit')
    #         else:
    #             ax.fill_between(x_fit, fit.p['mres'].mean-fit.p['mres'].sdev, fit.p['mres'].mean+fit.p['mres'].sdev, 
    #                             color='k',alpha=.3,label='fit')

    #         y  = [k.mean for k in mres]
    #         dy = [k.sdev for k in mres]
    #         if colors is not None:
    #             ax.errorbar(x,y,yerr=dy,linestyle='None',marker='s',mfc='None',color=colors, alpha=.5, label=ens)
    #         else: 
    ...
    #         ax.set_ylim(fp.corr_lst[k]['ylim'])
    ...
    # def plot_mres_avg(ax, dsets, key, mtype='exp', tau=1, colors=None, offset=0, denom_key=None,show_fit=True):
    #     Nt = 
    #     lst = [k for k in dsets if key in k]
    #     mres = lst[0].split('_')[0]
    #     x_fit =  np.arange(10,Nt//2+1)
    ...
    #     if show_fit:
    #         ax.fill_between(x_fit, fit.p['mres'].mean-fit.p['mres'].sdev, fit.p['mres'].mean+fit.p['mres'].sdev, 
    #                         color='b',alpha=.3,label='fit')
            

    # def make_mres_stability_plot(ax, dsets, key, mtype='exp', tau=1, colors=None, offset=0, denom_key=None):
    ...
    save_figs):
        ...
        for state in stability:
            ...
            ylim = None
            eff_plots.plot_stability(fits, tmin, n_states, tn_opt,
                                state, ylim=ylim, save=save_figs)
            if es_stability:
                for i_n in range(1, n_states[-1]):
                    eff_plots.plot_stability(fits, tmin, n_states, tn_opt, state,
